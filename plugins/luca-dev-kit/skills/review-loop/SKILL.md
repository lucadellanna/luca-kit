...
**STATUS: FIXED** -- copy the 64-character hex string from the sub-agent's `FIX_HASH: <value>` line (the hash only, no prefix or trailing text), then update state file atomically:
```bash
