...
| Memory move-outs auto-applied only when target path is under `./.claude/memory/`; global memory targets stay as advice | Project memory has an unambiguous destination scoped to this repo. Global memory requires confirming the rule's relevance across other projects: that is a user judgment, not an audit one. |
...
| Cache to fixed `/tmp/audit-claude-orig.md` (no path hashing) | The luca-ops-kit version hashes paths to avoid collision between e.g. `~/.claude/CLAUDE.md` and `./CLAUDE.md`. Single-file scope eliminates that risk. |
