...
| jq pre-aggregation extracts `.text` only for schema 1 (not `type+text`) | Prepending type to schema 1 text would break cross-schema recurrence: `"memory: X"` ≠ `"X"` in schema 2, suppressing valid cross-schema matches. Pre-aggregation is a candidate filter only; the model in Step 2 performs authoritative type-aware analysis on full entries. |
